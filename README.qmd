...
srn_distances_aggregated = srn_distances |>
  ...
  group_by(id) |>
  summarise(
    cycling_km_baseline = mean(cycling_km_baseline, na.rm = TRUE),
    cycling_km_go_dutch = mean(cycling_km_go_dutch, na.rm = TRUE),
    .groups = "drop"
  )
srn_joined = left_join(srn, srn_distances_aggregated, by = "id")
# tmap to plot:
m1 = tm_shape(srn_joined) +
  tm_lines(col = "cycling_km_go_dutch", title = "Go Dutch (km/day)")
m2 = tm_shape(srn_joined) +
  tm_lines("cycling_km_baseline", title = "Baseline (km/day)")
tmap_arrange(m1, m2)
```

Let's pre-process the roads into named groups, that we'll use to identify segments with high potential:
...
summary(sf::st_length(srn_joined))
    # Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
    # 0.02    45.42   142.10   388.39   405.19 19085.20 
summary(srn_joined$cycling_km_go_dutch)
  #  Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
  #   0.0   715.7  2445.1  5009.3  7171.3 50691.6   11102 
srn_filtered = srn_joined |>
  # filter(length_m > 200) |>
  filter(cycling_km_go_dutch > 2000) |>
  # Create categorical variable with cycling potential split into 10 categories:
  # filter(!is.na(name))
  # Quantiles of cycling potential:
  mutate(cycling_km_go_dutch_quantile = cut(cycling_km_go_dutch, breaks = quantile(cycling_km_go_dutch, probs = seq(0, 1, 0.1), na.rm = TRUE), include.lowest = TRUE, labels = FALSE)) 
summary(srn_filtered$cycling_km_go_dutch_quantile)
...
srn_grouped = srn_filtered |>
  ...
  mutate(Name = paste(road_classification_number, ifelse(is.na(name), "Unnamed", name), group, cycling_km_go_dutch_quantile, sep = " ")) |>
  ...
  arrange(desc(cycling_km_go_dutch)) |>
  mutate(id = as.character(row_number())) |>
  # Update id so that it's "" if there's only 1 in that group:
  mutate(id = ifelse(n() == 1, "", id)) |>
  mutate(Name = paste(road_classification_number, ifelse(is.na(name), "Unnamed", name), id, sep = " ")) |>
...
The top 100 roads are as follows:
...
#   head(10)
srn_top = srn_grouped |>
  select(Name, segment_length, cycling_km_baseline, cycling_km_go_dutch) |>
  head(100)
srn_top |>
  knitr::kable()
# srn_top$Name
# summary(sf::st_length(srn_top))
# summary(srn_top$cycling_km_go_dutch)
# summary(sf::st_length(srn_top))

# Split the SRN:

# summary(duplicated(srn_top$name))
pvars = c("Name", "cycling_km_go_dutch", "segment_length") 
m_top_srn = tm_shape(srn_filtered) +
  tm_lines(col = "darkgrey", lwd = 4) +
  tm_shape(srn_top) +
  tm_lines(col = "cycling_km_go_dutch", title = "Go Dutch (km/day)", lwd = 8, popup.vars = pvars, col.scale = tm_scale(breaks = c(10, 15, 25, 50) * 1000, values = "viridis"))
m_top_srn
tmap_save(m_top_srn, "m_top_srn.html")
system("gh release upload v1.0 m_top_srn.html --clobber")
```

The interactive map can be viewed by downloading the file at the following link: https://github.com/acteng/severance/releases/download/v1.0/m_top_srn.html
